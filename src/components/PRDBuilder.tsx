...
<head>
  ...
  <title>PRD - ${ideaData.productIdea}</title>
  <style>
    body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Arial, sans-serif; max-width: 800px; margin: 40px auto; padding: 20px; line-height: 1.6; color: #333; }
    h1 { color: #1a1a1a; border-bottom: 3px solid #4CAF50; padding-bottom: 10px; }
    h2 { color: #2c3e50; margin-top: 30px; border-left: 4px solid #4CAF50; padding-left: 15px; }
    .metadata { background: #f5f5f5; padding: 15px; border-radius: 8px; margin-bottom: 30px; }
    .metadata p { margin: 5px 0; }
    .section { margin-bottom: 40px; page-break-inside: avoid; }
    .footer { margin-top: 50px; padding-top: 20px; border-top: 2px solid #eee; text-align: center; color: #777; font-size: 14px; }
    hr { border: none; border-top: 1px solid #ddd; margin: 30px 0; }
    @media print { body { margin: 0; padding: 20px; } }
  </style>
</head>
<body>
  <h1>${ideaData.productIdea}</h1>
  
  <div class="metadata">
    <p><strong>Purpose:</strong> ${ideaData.purpose === 'deliverable' ? 'Company Deliverable' : 'Recruiting Portfolio'}</p>
    ${ideaData.persona ? `<p><strong>Role:</strong> ${ideaData.persona}</p>` : ''}
    ${ideaData.company ? `<p><strong>Company Context:</strong> ${ideaData.company}</p>` : ''}
  ...
  <div class="section">
    ...
    htmlContent += `
  <div class="footer">
    <p>Generated by Leland+ PRD Builder</p>
  </div>
</body>
</html>`;
    ...
    link.href = url;
    link.download = `PRD-${ideaData.productIdea.replace(/[^a-z0-9]/gi, '-').toLowerCase()}.html`;
    ...
    toast({
      title: "PRD Exported!",
      description: "Your PRD has been downloaded as an HTML document that you can open in any browser or convert to PDF.",
    });
  };
