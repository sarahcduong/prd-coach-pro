...
export const PRDBuilder = ({ ideaData, onBack }: PRDBuilderProps) => {
  ...
  const handleExport = () => {
    ...
    // Title
    addText(ideaData.productIdea, 20, true, [26, 26, 26]);
    yPosition += 10;
    ...
    doc.rect(margin, yPosition, maxWidth, metadataHeight, 'FD');
    yPosition += 8;

    addText(`Purpose: ${ideaData.purpose === 'deliverable' ? 'Company Deliverable' : 'Recruiting Portfolio'}`, 10);
    if (ideaData.persona) {
      yPosition += 2;
      addText(`Role: ${ideaData.persona}`, 10);
    }
    if (ideaData.company) {
      yPosition += 2;
      addText(`Company Context: ${ideaData.company}`, 10);
    }
    yPosition += 2;
    ...
    prdSections.forEach((section) => {
      ...
      if (content && content.trim()) {
        ...
        doc.setLineWidth(2);
        doc.line(margin, yPosition, margin + 5, yPosition);
        
        addText(section.title, 14, true, [44, 62, 80]);
        yPosition += 8;

        // Section content
        addText(content, 10, false, [51, 51, 51]);
        yPosition += 10;
    ...
    doc.setFontSize(9);
    doc.text('Generated by Leland+ PRD Builder', pageWidth / 2, yPosition, { align: 'center' });

    // Save the PDF
    doc.save(`PRD-${ideaData.productIdea.replace(/[^a-z0-9]/gi, '-').toLowerCase()}.pdf`);

    toast({
      title: "PRD Exported!",
      description: "Your PRD has been downloaded as a PDF document.",
    });
  };
